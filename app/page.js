...
const REPEAT_PRESETS = [
  { id: "none", label: "Немає" },
  { id: "day", label: "Щодня" },
  { id: "week", label: "Щотижня" },
  { id: "month", label: "Щомісяця" },
  { id: "year", label: "Щороку" },
  { id: "custom", label: "Кожні N…" },
];
const REPEAT_UNIT_OPTIONS = [
  { value: "day", label: "днів" },
  { value: "week", label: "тижнів" },
  { value: "month", label: "місяців" },
  { value: "year", label: "років" },
];
...
export default function CalendarPage() {
  ...
  const eventsByDate = useMemo(() => {
    ...
    for (const dateKey of Object.keys(groups)) {
      groups[dateKey].sort(
        (left, right) =>
          (left.time || "99:99").localeCompare(right.time || "99:99") || left.title.localeCompare(right.title, "uk"),
      );
    }
                    ...
                    <div className="event-content">
                      ...
                      {deletingId === event.id ? (
                        ...
                        >
                          <p>
                            {event.repeat
                              ? "Це повторювана справа. Прибрати лише цей день чи видалити всі повтори?"
                              : "Видалити цю справу? Вона піде до списку видалених разом з історією."}
                          </p>
                          <div className="delete-actions">
                            <button className="delete-confirm-button" type="submit" disabled={busy}>
                              {event.repeat ? "Лише цей день" : "Видалити"}
                            </button>
                            {event.repeat ? (
                              <button
                ...
                <div className="form-field">
                  ...
                  {form.repeat ? (
                    <label className="repeat-until">
                      Повторювати до (можна не вказувати)
                      <input
                        type="date"
                        min={form.date}
                  ...
                  {form.repeat && editingId ? (
                    <p className="repeat-hint">Зміни застосуються до всіх повторів цієї справи.</p>
                  ) : null}
                </div>
