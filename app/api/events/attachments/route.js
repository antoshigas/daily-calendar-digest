...
export async function POST(request) {
  try {
    ...
    if (file.size > MAX_ATTACHMENT_SIZE) {
      return jsonError("Файл завеликий. Ліміт 2 МБ.");
    }

    const fileType = file.type || "application/octet-stream";
    if (!ALLOWED_ATTACHMENT_TYPES.has(fileType)) {
      return jsonError("Такий тип файлу не підходить. Можна PDF, Word, Excel, txt і картинки.");
    }
    ...
    if ((target.attachments || []).length >= MAX_ATTACHMENTS_PER_EVENT) {
      return jsonError("До однієї справи можна прикріпити не більше 4 файлів");
    }
