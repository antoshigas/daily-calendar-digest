...
export const NO_TIME_LABEL = "без часу";
...
export function getPersonName(ownerId) {
  return PEOPLE.find((person) => person.id === normalizeOwnerId(ownerId))?.name || "Без власника";
}

export function getAccountName(accountId) {
  return ACCOUNTS.find((person) => person.id === normalizeAccountId(accountId))?.name || "Акаунт";
}
...
export function getDateLockReason(
  ...
  if (dateKey < todayKey) return "Цей день уже минув. Справи можна лише переглянути.";
  if (dateKey === todayKey && (todayLocked || todayAfterDigest)) {
    return "Розсилка на сьогодні вже пішла. Справи можна лише переглянути.";
  }
  return "";
}
...
export function formatRepeatLabel(repeat) {
  ...
  const base =
    repeat.interval === 1
      ...
      : `кожні ${repeat.interval} ${pluralForm(repeat.interval, REPEAT_UNIT_FORMS[repeat.unit])}`;

  if (!repeat.until) return base;

  const until = new Intl.DateTimeFormat("uk-UA", { day: "numeric", month: "numeric", year: "numeric" }).format(
  ...
  return [
    {
      ...
      summary: "Перенесено зі старої версії",
      changes: {},
    },
  ];
}
...
export function sortEvents(events) {
  return [...events].sort((left, right) => {
    ...
    const ownerCompare = getPersonName(left.ownerId).localeCompare(getPersonName(right.ownerId), "uk");
...
function formatAttachmentSummary(event) {
  ...
  return ` - документи: ${names}`;
}

export function buildTelegramMessage(events, now = new Date()) {
  const lines = [`Сьогодні, ${formatBerlinDay(now)}:`, ""];
