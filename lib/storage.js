...
const LOGIN_ATTEMPTS_KEY = "daily-calendar-login-attempts:v1";
const LOGIN_CHALLENGES_KEY = "daily-calendar-login-challenges:v1";
...
const LOCAL_LOGIN_ATTEMPTS_PATH = path.join(process.cwd(), "data", "login-attempts.local.json");
const LOCAL_LOGIN_CHALLENGES_PATH = path.join(process.cwd(), "data", "login-challenges.local.json");
...
function normalizeLoginAttempts(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};

  const now = Date.now();
  return Object.fromEntries(
    Object.entries(value)
      .filter(([attemptKey, attempt]) => attemptKey && attempt && typeof attempt === "object")
      .map(([attemptKey, attempt]) => {
        const lastFailedAt =
          typeof attempt.lastFailedAt === "string" && attempt.lastFailedAt
            ? attempt.lastFailedAt
            : new Date().toISOString();
        const lockedUntil = typeof attempt.lockedUntil === "string" ? attempt.lockedUntil : "";

        return [
          attemptKey,
          {
            accountId: normalizeAccountId(attempt.accountId),
            scope: typeof attempt.scope === "string" ? attempt.scope.slice(0, 32) : "client",
            failedCount: Number.isFinite(Number(attempt.failedCount)) ? Math.max(0, Number(attempt.failedCount)) : 0,
            firstFailedAt:
              typeof attempt.firstFailedAt === "string" && attempt.firstFailedAt ? attempt.firstFailedAt : lastFailedAt,
            lastFailedAt,
            lockedUntil,
          },
        ];
      ...
      .filter(([, attempt]) => {
        ...
        const lastFailedAt = Date.parse(attempt.lastFailedAt);
        if (Number.isFinite(lockedUntil) && lockedUntil > now) return true;
        return Number.isFinite(lastFailedAt) && now - lastFailedAt < LOGIN_ATTEMPT_RETENTION_MS;
      }),
  );
}

export async function readLoginAttempts() {
  const rawAttempts = getRedisConfig()
    ? JSON.parse((await redisCommand(["GET", LOGIN_ATTEMPTS_KEY])) || "{}")
    : await readLocalJson(LOCAL_LOGIN_ATTEMPTS_PATH, {});

  const normalized = normalizeLoginAttempts(rawAttempts);
  if (hasDifferentJson(rawAttempts, normalized)) {
    await writeStoredJson(LOGIN_ATTEMPTS_KEY, LOCAL_LOGIN_ATTEMPTS_PATH, normalized);
  }

  return normalized;
}

export async function writeLoginAttempts(attempts) {
  const normalized = normalizeLoginAttempts(attempts);
  await writeStoredJson(LOGIN_ATTEMPTS_KEY, LOCAL_LOGIN_ATTEMPTS_PATH, normalized);
  return normalized;
}

function normalizeLoginChallenges(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};

  const now = Date.now();
  return Object.fromEntries(
    Object.entries(value)
      .filter(([challengeId, challenge]) => challengeId && challenge && typeof challenge === "object")
      .map(([challengeId, challenge]) => [
        challengeId,
        {
          accountId: normalizeAccountId(challenge.accountId),
          salt: typeof challenge.salt === "string" ? challenge.salt : "",
          answerHash: typeof challenge.answerHash === "string" ? challenge.answerHash : "",
          createdAt:
            typeof challenge.createdAt === "string" && challenge.createdAt ? challenge.createdAt : new Date().toISOString(),
          expiresAt: typeof challenge.expiresAt === "string" ? challenge.expiresAt : "",
        },
      ])
      .filter(([, challenge]) => {
        const expiresAt = Date.parse(challenge.expiresAt);
        return challenge.salt && challenge.answerHash && Number.isFinite(expiresAt) && expiresAt > now;
      }),
  );
}

export async function readLoginChallenges() {
  const rawChallenges = getRedisConfig()
    ? JSON.parse((await redisCommand(["GET", LOGIN_CHALLENGES_KEY])) || "{}")
    : await readLocalJson(LOCAL_LOGIN_CHALLENGES_PATH, {});

  const normalized = normalizeLoginChallenges(rawChallenges);
  if (hasDifferentJson(rawChallenges, normalized)) {
    await writeStoredJson(LOGIN_CHALLENGES_KEY, LOCAL_LOGIN_CHALLENGES_PATH, normalized);
  }

  return normalized;
}

export async function writeLoginChallenges(challenges) {
  const normalized = normalizeLoginChallenges(challenges);
  await writeStoredJson(LOGIN_CHALLENGES_KEY, LOCAL_LOGIN_CHALLENGES_PATH, normalized);
  return normalized;
}
