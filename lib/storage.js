...
const STORAGE_KEY = `${KEY_PREFIX}daily-calendar-events:v1`;
const DIGEST_RUNS_KEY = `${KEY_PREFIX}daily-calendar-digest-runs:v1`;
const OWNER_PASSWORDS_KEY = `${KEY_PREFIX}daily-calendar-owner-passwords:v1`;
const DELETED_EVENTS_KEY = `${KEY_PREFIX}daily-calendar-deleted-events:v1`;
const SESSIONS_KEY = `${KEY_PREFIX}daily-calendar-sessions:v1`;
const LOGIN_ATTEMPTS_KEY = `${KEY_PREFIX}daily-calendar-login-attempts:v1`;
const GRAPHIC_KEYS_KEY = `${KEY_PREFIX}daily-calendar-graphic-keys:v1`;
...
const LOCAL_GRAPHIC_KEYS_PATH = path.join(process.cwd(), "data", "graphic-keys.local.json");
...
function normalizeGraphicKeys(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
...
export async function readGraphicKeys() {
  const rawKeys = getRedisConfig()
    ? JSON.parse((await redisCommand(["GET", GRAPHIC_KEYS_KEY])) || "{}")
    : await readLocalJson(LOCAL_GRAPHIC_KEYS_PATH, {});

  const normalized = normalizeGraphicKeys(rawKeys);
  if (hasDifferentJson(rawKeys, normalized)) {
    await writeStoredJson(GRAPHIC_KEYS_KEY, LOCAL_GRAPHIC_KEYS_PATH, normalized);
  }

  return normalized;
}

export async function writeGraphicKeys(keys) {
  const normalized = normalizeGraphicKeys(keys);
  await writeStoredJson(GRAPHIC_KEYS_KEY, LOCAL_GRAPHIC_KEYS_PATH, normalized);
  return normalized;
}
