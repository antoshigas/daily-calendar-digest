...
async function verifyLegacyPassword(accountId, password) {
  ...
  if (!record?.salt || !record?.hash) {
    throw new Error("Для цього акаунта пароль ще не заведено в БД");
  }
...
export async function requireSessionAccount(request) {
  ...
  if (!account) {
    const error = new Error("Потрібно увійти в акаунт");
    error.status = 401;
    throw error;
  }
...
function authError(message = "Не вдалося увійти", status = 401, retryAfter = 0) {
...
function normalizeStrictAccountId(accountId) {
  ...
  if (!ACCOUNTS.some((account) => account.id === rawAccountId)) {
    throw authError("Оберіть акаунт", 400);
  }
...
async function assertLoginAllowed(accountId, request) {
  ...
  if (Number.isFinite(lockedUntil) && lockedUntil > Date.now()) {
    ...
    throw authError(`Забагато спроб. Вхід закрито ще на ${Math.ceil(retryAfter / 60)} хв.`, 429, retryAfter);
  }
}
...
async function recordLoginFailure(accountId, request) {
  ...
  if (lockedUntil) {
    throw authError("Забагато спроб. Вхід закрито на 30 хв.", 429, Math.ceil(LOGIN_LOCK_MS / 1000));
  }
}
...
function cleanGraphicPattern(answer, { required = true } = {}) {
  ...
  if (
    ...
    throw authError("Графічний ключ має містити щонайменше 4 різні точки", 400);
  }
...
async function verifyGraphicKey(accountId, pattern) {
  ...
  if (expected.length !== actual.length || !timingSafeEqual(expected, actual)) {
    throw authError("Невірний вхід. Перевірте пароль і графічний ключ.");
  }
